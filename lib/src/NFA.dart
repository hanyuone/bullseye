...
class NFA {
  ...
  void _connectNode(List group, Node start, Node end) {
    ...
    if (group[0] == "STAR") {
      ...
      List items = group[1];
      int items_length = items.length - 1;
      List<Node> group_nodes = [start];
      this.node_index += items_length;

      for (int a = 0; a < items_length; a++) {
  ...
  void _parseRegex() {
    ...
    for (List group in this.regex_groups) {
      Node temp_node = new Node(this.node_index);
      this.ending_node = this.node_index;
      this.node_index++;

      this._connectNode(group, this.nodes[starting_index], temp_node);
